...
def init_db():
    ...
    defaults = {
        ...
        'nav_shop_ar':         'المتجر',
        'nav_about_ar':        'عن سلافة',
        'nav_contact_ar':      'تواصل معنا',
        # trust bar
        'trust1_ar':           '100% طبيعى',   'trust1_icon': '🌿',
        'trust2_ar':           'معصور على البارد', 'trust2_icon': '🫒',
        'trust3_ar':           'شحن مجانى فوق 500 جنيه', 'trust3_icon': '🚚',
        # benefits
        'benefits_title_ar':   'اكتشف فوائد زيوت سلافة',
        # shop
        'shop_title_ar':       'المتجر',
        'btn_add_ar':          'أضف للسلة',
        'filter_all_ar':       'الكل',
        'sidebar_cats_ar':     'الأصناف',
        # cart
        'cart_title_ar':       'سلة الشراء',
        'btn_checkout_ar':     'إتمام الشراء',
        'free_ship_text_ar':   'مجانى 🎉',
        # footer
        'footer_about_ar':     'أجود زيوت الزيتون من المزارع الأصيلة مباشرةً لمائدتك.',
        'footer_copy_ar':      'جميع الحقوق محفوظة',
        # newsletter btn
        'newsletter_btn_ar':   'اشترك',
        # brand sub
        'brand_sub':           'SULAFA OIL',
    }
    cur.executemany(
...
def save_uploaded_photo(file):
    """حفظ صورة مرفوعة — يقبل PNG · JPG · WEBP · GIF فقط."""
...
def save_uploaded_video(file):
    """حفظ فيديو مرفوع — يقبل MP4 · WEBM · OGG فقط."""
    if not file or not file.filename:
        return None
    ext = file.filename.rsplit('.', 1)[-1].lower()
...
def pay():
    ...
    # ── تكلفة الشحن الفعلية حسب المحافظة المختارة ──
    city = request.form.get('city', '')
    gov_ship_key = f'ship_gov_{city}' if city else None
    if gov_ship_key and s.get(gov_ship_key):
        ft   = float(s.get('free_shipping_threshold') or 500)
        ship = 0 if sub >= ft else float(s[gov_ship_key])
        grand = sub + tax + ship
...
def track():
    ...
    if not phone:
        return render_template('track.html', order=None, orders=None, history={}, error=False, s=s, lang=get_lang())
    ...
    if not orders:
        return render_template('track.html', order=None, orders=None, history={}, error=True, s=s, lang=get_lang())

    if order_number:
        order = next((o for o in orders if o['order_number'] == order_number), orders[0])
    else:
        order = orders[0]

    # جلب السجل التاريخى للطلب المحدد
    history_rows = conn.execute(
        'SELECT status, changed_at FROM order_status_history WHERE order_id=? ORDER BY id ASC',
        (order['id'],)
    ).fetchall()
    history = {r['status']: r['changed_at'][:16].replace('T', ' ') for r in history_rows}

    return render_template('track.html', order=order, orders=orders,
                           history=history, error=False, s=s, lang=get_lang())
...
def admin_dashboard():
    ...
    daily_revenues = [(r['day'][-5:], r['rev']) for r in daily_rows]
    ...
    # آخر 8 تغييرات حالة
    recent_status_changes = conn.execute(
        '''SELECT h.status, h.changed_at, o.order_number, o.full_name
...
def admin_orders_delete():
    """حذف طلب واحد أو أكثر مع order_items الخاصة بيهم."""
    ids = request.form.getlist('order_ids')
    if not ids:
        flash('لم يتم تحديد أى طلبات للحذف.', 'warning')
        return redirect(url_for('admin_orders'))

    # نتأكد إن القيم أرقام فقط — حماية من أى injection
    safe_ids = [int(i) for i in ids if i.isdigit()]
    if not safe_ids:
        return redirect(url_for('admin_orders'))
    ...
    # ١) حذف عناصر الطلبات أولاً (FK)
    conn.execute(f'DELETE FROM order_items WHERE order_id IN ({placeholders})', safe_ids)
    # ٢) حذف سجل تاريخ الحالات
    conn.execute(f'DELETE FROM order_status_history WHERE order_id IN ({placeholders})', safe_ids)
    # ٣) حذف الطلبات نفسها
    conn.execute(f'DELETE FROM orders WHERE id IN ({placeholders})', safe_ids)
    conn.commit()

    n = len(safe_ids)
    msg = 'تم حذف الطلب بنجاح ✅' if n == 1 else f'تم حذف {n} طلبات بنجاح ✅'
    flash(msg, 'success')
    return redirect(url_for('admin_orders'))
...
def admin_order_invoice(oid):
    # ✅ دالة نظيفة — بتعرض الفاتورة فقط بدون أي كود تاني
    conn  = get_db()
    order = conn.execute('SELECT * FROM orders WHERE id=?', (oid,)).fetchone()
    if not order:
        return redirect(url_for('admin_orders'))
...
def admin_settings():
    if request.method == 'POST':
        ...
        # ── 1. معالجة رفع الصور والفيديوهات ───────────────────
        media_fields = {
            'hero_image_file':     ('hero_image',     save_uploaded_photo),
            'hero_video_file':     ('hero_video',     save_uploaded_video),
            'about_image_file':    ('about_image',    save_uploaded_photo),
            'about_video_file':    ('about_video',    save_uploaded_video),
            'benefits_image_file': ('benefits_image', save_uploaded_photo),
            'benefits_video_file': ('benefits_video', save_uploaded_video),
            'promo_image_file':    ('promo_image',    save_uploaded_photo),
            'promo_video_file':    ('promo_video',    save_uploaded_video),
        }
        for field, (key, saver) in media_fields.items():
            f   = request.files.get(field)
            url = saver(f) if f and f.filename else None
            if url:
                conn.execute(
        ...
        # ── 3. حفظ باقي حقول النموذج النصية ───────────────────
        skip_keys = {k for k in request.form if k.startswith(('cat_ar_', 'cat_en_'))}
        conn.executemany(
            'INSERT OR REPLACE INTO settings (key, value) VALUES (?,?)',
            [(k, v) for k, v in request.form.items() if k not in skip_keys]
        )

        # ── 4. الـ checkboxes لما تكون مش محددة مش بتبعت قيمة — نحفظها صفر يدوياً ──
        checkbox_keys = ['card_payment_enabled']
        for ck in checkbox_keys:
            if ck not in request.form:
                conn.execute(
                    'INSERT OR REPLACE INTO settings (key, value) VALUES (?,?)',
                    (ck, '0')
                )
...
if __name__ == '__main__':
    ...
    ### ملحوظه لما بشغله اونولاين لازن اعمل دى 
# init_db()
##if __name__ == '__main__':
#print("✅  سلافة SULAFA  — http://localhost:5000")
# print("🔧  Admin Panel   — http://localhost:5000/admin")
