...
def track():
    ...
    if not phone:
        return render_template('track.html', order=None, orders=None, error=False, s=s, lang=get_lang())
    ...
    if not orders:
        return render_template('track.html', order=None, orders=None, error=True, s=s, lang=get_lang())

    # لو اختار طلب معين من القائمة
    if order_number:
        order = next((o for o in orders if o['order_number'] == order_number), orders[0])
    else:
        order = orders[0]

    return render_template('track.html', order=order, orders=orders, error=False, s=s, lang=get_lang())
